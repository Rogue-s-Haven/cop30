...
document.getElementById('onibus').addEventListener('change', function() {
  ...
  fetch(`http://localhost:8080/assentos`)
    ...
    .then(assentos => {
      // Filtra os assentos pelo 'onibusPlaca' selecionado e status 'disponivel'
      const assentosFiltrados = assentos.filter(assento => assento.onibusPlaca === onibusPlaca && assento.status === 'disponivel');

      if (assentosFiltrados.length === 0) {
        // Se não houver assentos disponíveis para o ônibus, exibe uma opção informativa
        const option = document.createElement('option');
        option.value = '';
        option.textContent = 'Nenhum assento disponível';
        assentoSelect.appendChild(option);
      } else {
        // Caso haja assentos disponíveis, preenche o select com os números dos assentos
